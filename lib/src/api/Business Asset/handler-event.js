...
const { updateRiskImpact, updateRiskName } = require('../Risk/handler-event');
const { updateBusinessAssetRef } = require('../Supporting Asset/handler-event');
...
const addBusinessAsset = (israProject, win) => {
  try {
    ...
    return dialog.showMessageBoxSync(getMainWindow(), { message: 'Failed to add business asset' });
  }
};
...
const deleteBusinessAsset = (israProject, ids, win) => {
  try {
    ...
    risks.forEach((risk)=>{
      const { riskId, businessAssetRef} = risk;
      if(deletedIds.has(String(businessAssetRef))){
        const affectedRisk = israProject.getRisk(riskId);
        affectedRisk.businessAssetRef = null;
        affectedRisk.supportingAssetRef = null;
    ...
    console.log(err)
    dialog.showMessageBoxSync(getMainWindow(), { message: 'Failed to delete business asset(s)' });
  }
};
...
const validateBusinessAsset = (israProject, data) => {
  try {
    ...
    dialog.showMessageBoxSync(getMainWindow(), { message: 'Failed to validate Business Assets tab' });
  }
};
...
const updateBusinessAsset = (ISRAproject, win, id, field, value) => {
  try {
    ...
    if (field === 'businessAssetName' || field === 'businessAssetType') {
      ...
      const risks = israProject.properties.Risk;
        risks.forEach((risk) => {
          if(risk.businessAssetRef === id) updateRiskImpact(israProject, risk.riskId);
      });
    }
    // win.webContents.send('risks:load', israProject.toJSON());
  } catch (err) {
    dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to update business asset ${id}` });
  }
};
