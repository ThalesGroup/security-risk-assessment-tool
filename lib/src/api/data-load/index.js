...
const { sanitizeSupportingAssetRefs, sanitizeTrackingURI, sanitizeTrackingURIWithReport } = require('../xml-json/alter-isra/sanitizers');
...
const DataLoad = (filePath) => {
  try {
    ...
    jsonData.Vulnerability.forEach((vulnerability) => {
      ...
      vulnerability.supportingAssetRef = sanitizeSupportingAssetRefs(
        vulnerability.supportingAssetRef
      );

      const { sanitized, wasInvalid, original } = sanitizeTrackingURIWithReport(
        vulnerability.vulnerabilityTrackingURI
      );
      vulnerability.vulnerabilityTrackingURI = sanitized;
      if (wasInvalid) {
        sanitizationReport.push({
    ...
    // Side-channel for the main process to surface wiped URIs to the user.
    // Not a private field on ISRAProject so it isn't included in toJSON().
    israProject.sanitizationReport = sanitizationReport;
    return israProject;
  } catch (error) {
