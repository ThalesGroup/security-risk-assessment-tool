...
const { sanitizeSupportingAssetRefs, sanitizeTrackingURIWithResult } = require('../xml-json/alter-isra/sanitizers');
...
const DataLoad = (filePath) => {
  try {
    ...
    const validSupportingAssetIds = (jsonData.SupportingAsset || [])
      .filter(sa => sa && typeof sa === 'object' && sa.supportingAssetId)
      .map(sa => sa.supportingAssetId);

    const cleanupLog = [];

    jsonData.Vulnerability.forEach((vulnerability) => {
      if (!vulnerability || typeof vulnerability !== 'object') return;

      const sanitizeResult = sanitizeSupportingAssetRefs(
        vulnerability.supportingAssetRef,
        validSupportingAssetIds
      );

      if (sanitizeResult.removedCount > 0) {
        cleanupLog.push({
          type: 'supportingAssetRef',
          vulnerabilityId: vulnerability.vulnerabilityId,
          removedCount: sanitizeResult.removedCount
        });
      }

      vulnerability.supportingAssetRef = sanitizeResult.refs;

      const trackingURIResult = sanitizeTrackingURIWithResult(
        vulnerability.vulnerabilityTrackingURI
      );
      vulnerability.vulnerabilityTrackingURI = trackingURIResult.uri;

      if (trackingURIResult.removed) {
        cleanupLog.push({
          type: 'vulnerabilityTrackingURI',
          vulnerabilityId: vulnerability.vulnerabilityId,
          originalValue: trackingURIResult.original
        });
      }
    });
    ...
    return {
      project: israProject,
      cleanupLog: cleanupLog.length > 0 ? cleanupLog : null
    };
  } catch (error) {
