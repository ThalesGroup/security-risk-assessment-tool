...
const renderWelcome = () => {
  const projectOrganization = config.organizations;
  let projectOrganizationOptions = '';
  projectOrganizationOptions += `<option value="">Select...</option>`;
  projectOrganization.forEach((org) => {
    projectOrganizationOptions += `<option value="${org}">${org}</option>`;
  });
  const levelsOfTrust = config.levelsOfTrust;
  let levelsOfTrustOptions = '';
  levelsOfTrustOptions += `<option value="">Select...</option>`;
  levelsOfTrust.forEach((level) => {
    levelsOfTrustOptions += `<option value="${level}">${level}</option>`;
  });
  const html = `
  <div class="details">
    <header>${jsonSchema.title}</header>
    <p id="details__app-version">App Version: ${config.appVersion}</p>
  </div>
  <div id="welcome__isra-meta">
    ...
    <select name="welcome__isra-meta--organization" id="welcome__isra-meta--organization" required title="${ISRAmetaSchema.projectOrganization.description}">
      ${projectOrganizationOptions}
    </select>
    <label for="welcome__isra-meta--targeted-level-of-trust">${ISRAmetaSchema.targetedLevelOfTrust.title}</label>
    <select name="welcome__isra-meta--targeted-level-of-trust" id="welcome__isra-meta--targeted-level-of-trust" title="${ISRAmetaSchema.targetedLevelOfTrust.description}">
      ${levelsOfTrustOptions}
    </select>
  </div>
    ...
    <section>
      ...
      <p>The threat modeling can be done with or without an external tool. The decision to use a threat modeling tool depends on 2 criteria:
        <ul>
          ...
          <li>the result of the Targeted level of trust assessment (as defined in ISO 27034): if the TLOT falls within a certain minimum threshold it may not be necessary to use a threat modeling tool.</li>
        </ul>
      </p>
  ...
  const tableOptions = {
    ...
    columns: [ // Define Table Columns
      ...
      {
        title: `${ISRATrackingSchema.trackingIteration.title}`, field: 'trackingIteration', width: 100, headerSort: false, headerTooltip: `${ISRATrackingSchema.trackingIteration.description}`, headerHozAlign: 'center', validator: 'integer',
      },
      {
        title: `${ISRATrackingSchema.trackingSecurityOfficer.title}`, field: 'trackingSecurityOfficer', editor: 'input', headerSort: false, headerTooltip: `${ISRATrackingSchema.trackingSecurityOfficer.description}`, headerHozAlign: 'center',
      },
      {
      ...
      {
        title: `${ISRATrackingSchema.trackingComment.title}`, field: 'trackingComment', headerSort: false, headerTooltip: `${ISRATrackingSchema.trackingComment.description}`, headerHozAlign: 'center',
      },
    ],
  };
