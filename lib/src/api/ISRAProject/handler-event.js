...
const addTrackingRow = (israProject) => {
  try {
    ...
    israProject.iteration = israProject.properties.ISRAmeta.ISRAtracking.length
    return israTracking.properties;
  } catch (err) {
    console.log(err)
    return dialog.showMessageBoxSync(getMainWindow(), { message: 'Failed to add iteration row' });
  }
};
...
const deleteTrackingRow = (israProject, iterations) => {
  try {
    ...
    console.log(err)
    return dialog.showMessageBoxSync(getMainWindow(), { message: 'Failed to delete iteration row(s)' });
  }
};
...
const updateTrackingRow = (israProject, rowData) => {
  try {
    ...
    dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to update iteration row ${rowData.trackingIteration}` });
  }
};
...
const updateProjectNameAndVersionRef = (israProject, field, value) => {
  try {
    ...
    if (field === 'projectName') israProject.projectName = value;
    else if (field === 'projectVersion') israProject.projectVersion = value;
    Risk.forEach((r) => {
    ...
    dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to update project name/version of Risk/Vulnerability` });
  }
};
...
const validateISRAmeta = (israProject, data) => {
  try {
    const object = {
      ...
      targetedLevelOfTrust: data[2],
      projectVersion: data[3],
    };
    Object.assign(israProject, object);
  } catch (err) {
    console.log(err)
    dialog.showMessageBoxSync(getMainWindow(), { message: 'Failed to validate Welcome tab' });
  }
};
