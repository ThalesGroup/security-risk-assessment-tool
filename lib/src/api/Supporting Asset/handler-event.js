...
const addSupportingAsset = (israProject) => {
  try {
    ...
    return dialog.showMessageBoxSync(getMainWindow(), { message: 'Failed to add supporting asset' });
  }
};
...
const deleteSupportingAsset = (israProject, ids, win) => {
  try {
    ...
    risks.forEach((risk)=>{
      const { riskId, supportingAssetRef } = risk;
      if(deletedIds.has(String(supportingAssetRef))){
...
const deleteBusinessAssetRef = (israProject, id, indexes, win) => {
  try {
    ...
    const risks = israProject.properties.Risk;
    risks.forEach((risk) => {
      const { riskId, businessAssetRef, supportingAssetRef} = risk;    
      if (deletedBusinessAssetRefs.includes(businessAssetRef) && id === supportingAssetRef) {
        const affectedRisk = israProject.getRisk(riskId);
...
const updateBusinessAssetRef = (israProject, id, value, index, win) => {
  try {
    ...
    risks.forEach((risk) => {
      const { riskId, businessAssetRef, supportingAssetRef } = risk;
      if (!businessAssetRefs.includes(businessAssetRef) && id === supportingAssetRef) {
        const affectedRisk = israProject.getRisk(riskId);
        affectedRisk.businessAssetRef = null;
        affectedRisk.supportingAssetRef = null;
        updateRiskName(israProject, win, riskId);
