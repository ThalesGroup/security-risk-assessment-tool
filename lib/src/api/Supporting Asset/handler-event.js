...
const { updateVulnerability } = require('../Vulnerability/handler-event');
...
const addSupportingAsset = (israProject) => {
  try {
    ...
    return dialog.showMessageBox(currentWin, { message: 'Failed to add supporting asset' });
  }
};
...
const deleteSupportingAsset = (israProject, ids, win) => {
  try {
    ...
    console.log(err);
    dialog.showMessageBox(currentWin, { message: 'Failed to delete supporting asset(s)' });
  }
};
...
const updateSupportingAsset = (israProject, win, id, field, value) => {
  try {
    ...
    console.log(err)
    dialog.showMessageBox(currentWin, { message: `Failed to update supporting asset ${id}` });
  }
};
...
const deleteBusinessAssetRef = (israProject, id, indexes, win) => {
  try {
    ...
    risks.forEach((risk) => {
      const { riskId, riskName } = risk;
      if (!businessAssetRefs.includes(riskName.businessAssetRef)) {
        const affectedRisk = israProject.getRisk(riskId);
    ...
    console.log(err);
    dialog.showMessageBox(currentWin, { message: 'Failed to delete Business Asset Ref' });
  }
}
...
const updateBusinessAssetRef = (israProject, id, value, index, win) => {
  try {
    ...
    console.log(err)
    dialog.showMessageBox(currentWin, { message: 'Failed to update Business Asset Ref' });
  }
};
...
const validateSupportingAssets = (ISRAproject, supportingAssets, desc) => {
  try {
    ...
    dialog.showMessageBox(currentWin, { message: 'Failed to validate Supporting Assets tab' });
  }
};
