...
const xml1 = require('fs').readFileSync('./lib/test/test-2.xml', 'utf8');
  ...
  parseString(
    ...
    async (xmlErr, result) => {
      if (result === undefined) throw new Error(`XML not well formed at \nXML ${xmlErr}`);
        ...
        const israJSONData = await alterISRA(result.ISRA, xmlData);
        const israValidJSONData = await validateJsonSchema(israJSONData);
        const israProject = populateClass(israValidJSONData).getISRAProject();
