...
/** modify parsed ISRA JSON to fit schema
       ...
       * */
const alterSupportingAssets = (supportingAssetsCopy, businessAssetsCopy = []) => {
  // remove businessAssetRef(s) that do no exist if no match is found
  const businessAssetIds = new Set();
  businessAssetsCopy.forEach((asset)=>{
    businessAssetIds.add(asset.businessAssetId[0]);
  });
    ...
    supportingAssetsCopy.forEach((asset) => {
      ...
      if (supportingAndBusinessAssetLink !== undefined) {
        const businessAssetRef = supportingAndBusinessAssetLink.map((link) => {
          if (link.$.businessAssetRef !== '' && businessAssetIds.has(link.$.businessAssetRef)) return link.$.businessAssetRef;
          return null;
        });
        sa.businessAssetRef = businessAssetRef;
      }
    });
  }
};
