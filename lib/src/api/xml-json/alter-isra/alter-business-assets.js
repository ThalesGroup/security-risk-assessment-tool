...
const alterBusinessAssets = (xmlData, businessAssets) => ((xmlDataCopy, businessAssetsCopy) => {
  if (Array.isArray(businessAssetsCopy)) {
    const businessAssetCounter = utilityGlobal().counter();
    businessAssetsCopy.forEach((asset) => {
      businessAssetCounter.increment();
      const ba = asset;

      ba.businessAssetDescription = utility().getHTMLString(xmlDataCopy, 'businessAssetDescription', businessAssetCounter.getCount());
