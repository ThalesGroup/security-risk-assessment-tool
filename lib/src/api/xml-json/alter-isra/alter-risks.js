...
const alterRisks = (xmlDataCopy, risksCopy, businessAssetsCopy = [], supportingAssetsCopy = []) => {
  if (Array.isArray(risksCopy)) {
    ...
    risksCopy.forEach((risk) => {
      ...
      r.threatAgent = threatAgent[0] === 'Operationnal Employee' ? ['Operational Employee'] : threatAgent
      r.threatAgentDetail = getHTMLString(xmlDataCopy, 'threatAgentDetail', riskCounter.getCount())
      r.threatVerbDetail = getHTMLString(xmlDataCopy, 'threatVerbDetail', riskCounter.getCount())
      ...
      /* r.riskName = {
        // riskIdRef: riskId,
        riskName,
        threatAgent: threatAgent[0] === 'Operationnal Employee' ? ['Operational Employee'] : threatAgent,
      ...
      if('ComputedRiskName' in RiskName[0]){
        r.isAutomaticRiskName = true;

        // alter computed riskName if businessAssetId does not exist
        if (businessAssets[businessAssetRef[0]] === undefined) {
          let prevRiskName = r.riskName[0];
          prevRiskName = prevRiskName.replace(businessAssets[businessAssetRef[0]], '');
          r.riskName = prevRiskName.replace(supportingAssets[supportingAssetRef[0]], '');
        }
      }else r.isAutomaticRiskName = false;
      ...
      if (VulnerabilityRiskEvaluation !== undefined) {
        ...
        r.riskAttackPaths = VulnerabilityRiskEvaluation.map((attackPath) => {
          ...
          const vulnerabilityRefs = vulnerabilityRef.filter(uncheckedV => uncheckedV.$.name).map((v) => {
            const obj = {};
            obj.vulnerabilityId = v.char;
            obj.score = v.$.score;
            obj.name = v.$.name;
            return obj;
      ...
      if (Mitigation !== undefined) {
        ...
        r.mitigationsBenefits = mitigationsBenefits;
        r.mitigationsDoneBenefits = mitigationsDoneBenefits;
    });
  }
};
