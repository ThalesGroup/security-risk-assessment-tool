...
const { getHTMLString, getDecodedXMLFileAttachment } = require('../utility');
...
const alterProjectContext = (xmlData, projectContext) => {
  ...
  Object.entries(pc[0]).forEach((entry) => {
    ...
    if (key === 'projectDescriptionAttachment' && value[0] !== '') {
      pc[0][key] = getDecodedXMLFileAttachment(value[0]);
    }
  });
