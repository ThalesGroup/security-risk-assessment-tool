...
const XML2JSON = (filePath) => {
  ...
  if (resultJSON.cairis_model) {
    ...
    const validAssets = {"Information" : "","Systems": "Computer","Software" : "Software application","Hardware" : "Hardware device", "Person" : "Human"};
    const assets = resultJSON.cairis_model.riskanalysis.asset;
    console.log(assets)
    const risks = resultJSON.cairis_model.riskanalysis.risk;
    const mitigation = resultJSON.cairis_model.riskanalysis.response;
    const businessAssetRef = resultJSON.cairis_model.riskanalysis.asset_association;
    const vulnerabilities = resultJSON.cairis_model.riskanalysis.threat;
    let highestSAId = 0;
    let highestBAId = 0;
    const baIdMap = {};
    const saIdMap = {};
    assets.forEach((asset) => {
      ...
      if (validAssets.includes(asset._type)) {
        // Business Asset
        if (asset._type === 'Information') {
          highestBAId += 1
          const businessAsset = new BusinessAsset();
          businessAsset.businessAssetId = highestBAId;
          businessAsset.businessAssetName = asset._name;
          // Update baIdMap
          businessAsset.businessAssetType = validAssets[asset._type];
          businessAsset.businessAssetDescription = asset._description;
          //businessAsset.businessAssetProperties = asset._name; Loop through asset._security_property and add to businessAssetProperties object
        ...
        // Supporting Asset
          ...
          // Update saIdMap
          supportingAsset.supportingAssetName = asset._name;
          supportingAsset.supportingAssetType = validAssets[asset._type];
    ...
    vulnerabilities.forEach((vulnerability) => {
      ...
      newVulnerability.vulnerabilityName = vulnerability._name;
      newVulnerability.vulnerabilityDescription = vulnerability.description;
      newVulnerability.vulnerabilityFamily = vulFamilyMap[vulnerability._type]
