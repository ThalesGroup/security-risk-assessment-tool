...
const XML2JSON = (filePath) => {
  ...
  const israJSONData = alterISRA(resultJSON.ISRA, xmlData, sanitizationReport);
  ...
  // Side-channel: surface URIs that the loader silently wiped so the main
  // process can show a dialog. Not a private field — won't be serialized by
  // israProject.toJSON() (which only emits this.properties).
