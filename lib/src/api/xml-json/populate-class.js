...
const populateClass = (data, israProject) => {
  const {
    ISRAtracking, projectName, projectOrganization, targetedLevelOfTrust = '',projectVersion, iteration, latestBusinessAssetId, latestSupportingAssetId, latestRiskId, latestVulnerabilityId
  } = data.ISRAmeta;
  ...
  BusinessAsset.setIdCount(latestBusinessAssetId < highestBAId ? highestBAId : latestBusinessAssetId);
  SupportingAsset.setIdCount(latestSupportingAssetId < highestSAId ? highestSAId : latestSupportingAssetId);
  Risk.setIdCount(latestRiskId < highestRiskId ? highestRiskId : latestRiskId);
  Vulnerability.setIdCount(latestVulnerabilityId < highestVulId ? highestVulId : latestVulnerabilityId);

  // populate ISRAProject (meta)
  Object.assign(israProject, { projectName, projectOrganization, targetedLevelOfTrust, projectVersion, classification, iteration, latestBusinessAssetId, latestSupportingAssetId, latestRiskId, latestVulnerabilityId });
};
