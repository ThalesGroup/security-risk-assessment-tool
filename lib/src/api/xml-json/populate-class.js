...
const populateClass = (data, israProject) => {
  const {
    ISRAtracking, projectName, projectOrganization, projectVersion, iteration
  } = data.ISRAmeta;
  ...
  data.Risk.forEach((risk) => {
    if(risk.riskId){
      ...
      riskIds.push(riskId);
    } else unlabeledRisks.push(risk);
  });

  let highestRiskId = riskIds.length > 0 ? Math.max(...riskIds) : 0;

  unlabeledRisks.forEach((risk) => {
    const {
      projectNameRef, projectVersionRef,
      riskId, riskName, riskLikelihood, riskImpact, riskAttackPaths, riskMitigation, ...restRisk
    } = risk;
  ...
  Object.assign(israProject, { projectName, projectOrganization, projectVersion, iteration });
};
