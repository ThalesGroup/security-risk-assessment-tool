...
const populateClass = (data, israProject) => {
  ...
  data.Vulnerability.forEach((vulnerability) => {
    const {
      vulnerabilityId, supportingAssetRef, projectName, projectVersion, overallScore, ...rest
    } = vulnerability;
    var { cveScore } = vulnerability;
    israProject.projectName = projectName;
    israProject.projectVersion = projectVersion;
    const v = new Vulnerability();
    console.log(cveScore,overallScore)
    if (cveScore === 0 && overallScore !=0 ) {
      cveScore = overallScore;
    }

    Object.assign(v, { vulnerabilityId, cveScore: cveScore, ...rest });
