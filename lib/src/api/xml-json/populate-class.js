...
const populateClass = (data, israProject) => {
  const {
    ISRAtracking, projectName, projectOrganization, projectVersion, targetedLevelOfTrust, iteration, latestBusinessAssetId, latestSupportingAssetId, latestRiskId, latestVulnerabilityId
  } = data.ISRAmeta;
  ...
  data.Vulnerability.forEach((vulnerability) => {
    ...
    if (vulnerability.vulnerabilityId && !vulIds.includes(vulnerability.vulnerabilityId)){
      ...
      const validRefs = supportingAssetRef.filter(ref => saIds.includes(ref));
      validRefs.forEach((ref) => {
        v.addSupportingAssetRef(ref);
      });

      israProject.addVulnerability(v);
      vulIds.push(vulnerabilityId);
  ...
  Object.assign(israProject, { projectName, projectOrganization, projectVersion, targetedLevelOfTrust: ['1', '2', '3', '4'].includes(targetedLevelOfTrust) ? targetedLevelOfTrust : '1', classification, iteration, latestBusinessAssetId, latestSupportingAssetId, latestRiskId, latestVulnerabilityId });
};
