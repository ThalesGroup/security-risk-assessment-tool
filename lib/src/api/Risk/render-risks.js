...
const renderRisks = () => {
  ...
  threatAgentOptions = addEnumOptions(riskJsonSchema.threatAgent.enum);
  threatVerbOptions = addEnumOptions(riskJsonSchema.threatVerb.enum);
  ...
  const html = `
    ...
    <div class='filter risks-filter'>
        <div class="risks-filter__search">
            <input id="filter-value" type="text" placeholder="Filter...">
            <button id="filter-clear">X</button>
        </div>
    </div>
    <div class="risks-sort-row">
        <div class="risks-filter__sort">
                <label for="sort-risk">Sort by</label>
                <select id="sort-risk">
                    <option value="id-asc" selected>Risk Id: Ascending</option>
                    <option value="id-desc">Risk Id: Descending</option>
                    <option value="level-asc">Risk Level: Low to High</option>
                    <option value="level-desc">Risk Level: High to Low</option>
                </select>
        </div>
    </div>
    <div class="table">
    ...
    <section id="risks__risk__description">
        ...
        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 2%;">
            <input type="checkbox" id="risk__isTransferredFromDependency" name="risk__isTransferredFromDependency" style="width: auto; margin: 0;">
            <label for="risk__isTransferredFromDependency" style="font-weight: bold; margin: 0; cursor: pointer;">Risk transferred from a dependency</label>
        </div>

        <div id="risk__dependencyOriginContainer" style="display: none; flex-direction: column; margin-bottom: 2%;">
            <label for="risk__dependencyOrigin" style="font-weight: bold; margin-bottom: 5px;">Comment for its origin / Dependency Name</label>
            <input type="text" id="risk__dependencyOrigin" name="risk__dependencyOrigin" placeholder="e.g. Third-party auth service" style="width: 100%; box-sizing: border-box; padding: 5px;">
        </div>

        <div id="risk__transferredScoreContainer" style="display: none; flex-direction: column; margin-bottom: 2%;">
            <label for="risk__transferredScore" style="font-weight: bold; margin-bottom: 5px;">Dependency Risk Score (0-20)</label>
            <input type="number" id="risk__transferredScore" name="risk__transferredScore" min="0" max="20" placeholder="e.g. 12" style="width: 100px; padding: 5px;">
        </div>
    
        <section id="risk__automatic_riskname">
            <div>
                <p><strong style="font-family: Arial, Helvetica, sans-serif; font-size: 1.5em; font-style: normal;">Risk Name:</strong> As a <strong>Threat Agent</strong>, I can <strong>Threat</strong> the <strong>Business Asset</strong> 
            ...
            <div class="risk__flex">
                <div>
                    ...
                    <select id="risk__threatAgent" name="risk__threatAgent" title="${riskJsonSchema.threatAgent.description}" required>
                        ${threatAgentOptions}
                    </select>
                ...
                <div>
                    ...
                    <select id="risk__threat" name="risk__threat" title="${riskJsonSchema.threatVerb.description}" required>
                        ${threatVerbOptions}
                    </select>
            ...
            <div class="risk__flex">
                <div>
                    ...
                    <select id="risk__businessAsset" name="risk__businessAsset" title="${riskJsonSchema.businessAssetRef.description}" required></select>
                </div>
                <div>
                    <label for="risk__supportingAsset">Compromising the <span>Supporting Asset</span></label>
                    <select id="risk__supportingAsset" name="risk__supportingAsset" title="${riskJsonSchema.supportingAssetRef.description}" required></select>
                </div>
            </div>
            ...
            <div>
                <div style="display: flex; flex-direction: column;">
                    ...
                    <input type="text" name="risk__motivation" id="risk__motivation" title="${riskJsonSchema.motivation.description}" required>
                    ...
                    <table style="border: 0;">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="background-color: var(--severity-medium-background); color: var(--severity-medium-contrast);"><span>Medium</span></td>
                                <td style="background-color: var(--severity-high-background); color: var(--severity-high-contrast);"><span>High</span></td>
                                <td style="background-color: var(--severity-very-high-background);"><span>Very High</span></td>
                                <td style="background-color: var(--severity-very-high-background);"><span>Very High</span></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td style="background-color: var(--severity-low-background); color: var(--severity-low-contrast);"><span>Low</span></td>
                                <td style="background-color: var(--severity-medium-background); color: var(--severity-medium-contrast);"><span>Medium</span></td>
                                <td style="background-color: var(--severity-medium-background); color: var(--severity-medium-contrast);"><span>Medium</span></td>
                                <td style="background-color: var(--severity-high-background); color: var(--severity-high-contrast);"><span>High</span></td>
                            </tr>
  ...
  const tableOptions = {
    ...
    columns: [ // Define Table Columns
      ...
      {
        title: 'Mgmt Decision', field: 'riskManagementDecision', headerSort: false, headerHozAlign: 'center', hozAlign: 'center', headerWordWrap: true
      },
    ],
  };
