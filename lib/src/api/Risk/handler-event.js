...
const calculateAttackPathScore = (riskAttackPath, win, israProject) => {
    const overallScores = [];
    riskAttackPath.properties.vulnerabilityRef.forEach((ref) => {
        overallScores.push(ref.score);
    });

    if (overallScores.includes(null) || overallScores.length === 0) riskAttackPath.attackPathScore = null;
...
const updateRiskAttackPath = (israProject, riskId, riskAttackPathId, rowid, field, value, win) => {
    try {
        ...
        if (field === 'vulnerabilityIdRef') {
            ...
            calculateAttackPathScore(riskAttackPath, win, israProject);
        }
    } catch (err) {
        console.log(err)
