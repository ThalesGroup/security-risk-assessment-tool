...
const addRiskAttackPath = (israProject, riskId, win) => {
    try {
        ...
        calculateAllAttackPathsName(risk, israProject);
        if(win) win.webContents.send('risks:load', israProject.toJSON());
    } catch (err) {
        console.log(err)
            ...
            for (let i = 0; i < riskAttackPath.properties.vulnerabilityRef.length; i++) {
                if (riskAttackPath.properties.vulnerabilityRef[i].score === null &&
                    riskAttackPath.properties.vulnerabilityRef[i].vulnerabilityIdRef !== null) 
                    {
                        ...
                        return riskAttackPath.riskAttackPathId;
                    }
            }
...
const calculateMitigationsBenefits = (risk) => {
    const { riskMitigation } = risk.properties;
...
/**
  ...
  * @param {integer} riskId current riskId
*/
const addRiskMitigation = (israProject, riskId) => {
    try {
        const risk = israProject.getRisk(riskId);
        ...
        console.log(err)
        return dialog.showMessageBoxSync(null, { message: `Failed to add Risk Mitigation for Risk ${riskId}` });
    }
};
...
const deleteRiskMitigation = (israProject, riskId, ids, win) => {
    try {
        ...
        console.log(err)
        return dialog.showMessageBoxSync(null, { message: `Failed to delete Risk Mitigation(s) for Risk ${riskId}` });
    }
};
...
*/
const updateRiskMitigation = (israProject, riskId, riskMitigationId, field, value) => {
    try {
        const risk = israProject.getRisk(riskId)
        ...
        console.log(err)
        return dialog.showMessageBoxSync(null, { message: `Failed to update Risk Mitigation ${riskMitigationId} for Risk ${riskId}` });
    }
}; 
...
/**
  ...
  * @param {} value to of updating field
*/
const updateRiskManagement = (israProject, riskId, field, value) => {
    try {
        const risk = israProject.getRisk(riskId);
        risk[field] = value;
        calculateResidualRiskScore(risk);
        ...
        console.log(err);
        return dialog.showMessageBoxSync(null, { message: `Failed to update Risk Management for Risk ${riskId}` });
    }
};
...
const validateRisks = (israProject, data) => {
    try {
        const { riskId, riskName, riskLikelihood, riskMitigation } = data;
        ...
        console.error(err)
        dialog.showMessageBoxSync(null, { message: 'Failed to validate Risk tab' });
    }
};
