...
const { log, isLoggingEnabled } = require('../../../../app/src/electron/logger');
...
const addRisk = (israProject) =>{
    try {
        ...
        return dialog.showMessageBoxSync(getMainWindow(), { message: 'Failed to add Risk' });
    }
};
...
const deleteRisk = (israProject, ids) => {
    try {
      ...
      dialog.showMessageBoxSync(getMainWindow(), { message: 'Failed to delete risk(s)' });
    }
};
...
const cloneRisk = (israProject, riskId) => {
    try {
        const sourceRisk = israProject.getRisk(riskId);
        const {
            riskId: sourceRiskId,
            riskLikelihood,
            riskImpact,
            ...
            riskMitigation,
            ...restRisk
        } = sourceRisk.properties;

        const clonedRisk = new Risk();

        Object.entries(restRisk).forEach(([key, value]) => {
            clonedRisk[key] = value;
        });

        const clonedRiskLikelihood = new RiskLikelihood();
        Object.entries(riskLikelihood).forEach(([key, value]) => {
            clonedRiskLikelihood[key] = value;
        });
        clonedRisk.riskLikelihood = clonedRiskLikelihood;

        const clonedRiskImpact = new RiskImpact();
        Object.entries(riskImpact).forEach(([key, value]) => {
            clonedRiskImpact[key] = value;
        });
        clonedRisk.riskImpact = clonedRiskImpact;

        riskAttackPaths.forEach((path) => {
            const clonedPath = new RiskAttackPath();
            clonedPath.attackPathName = path.attackPathName;
            clonedPath.attackPathScore = path.attackPathScore;
            path.vulnerabilityRef.forEach((ref) => {
                clonedPath.addVulnerability({ ...ref });
            });
            clonedRisk.addRiskAttackPath(clonedPath);
        });

        riskMitigation.forEach((mitigation) => {
            const clonedMitigation = new RiskMitigation();
            clonedMitigation.description = mitigation.description;
            clonedMitigation.benefits = mitigation.benefits;
            clonedMitigation.cost = mitigation.cost;
            clonedMitigation.decision = mitigation.decision;
            clonedMitigation.decisionDetail = mitigation.decisionDetail;
            clonedRisk.addRiskMitigation(clonedMitigation);
        });

        israProject.addRisk(clonedRisk);
        return [clonedRisk.properties, israProject.properties.Risk];
    } catch (err) {
        console.log(err);
        return dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to clone Risk ${riskId}` });
    }
};
...
const constructRiskName = (risk, israProject, field = 'updateRiskName') => {
    ...
    if (risk.businessAssetRef !== null) businessAsset = israProject.getBusinessAsset(risk.businessAssetRef);
    updateRiskImpact(israProject, risk.riskId);
    
    if (risk.supportingAssetRef !== null) supportingAsset = israProject.getSupportingAsset(risk.supportingAssetRef);

    if (field !== 'riskName') {
        if (field === 'automatic riskName') risk.isAutomaticRiskName = true;
        
        if (risk.isAutomaticRiskName){
            risk.riskName = 'As a ' + risk.threatAgent + ', I can ' + risk.threatVerb + ' the ' + (businessAsset === null ? '' : businessAsset.businessAssetName) + ' compromising the ' + (supportingAsset === null ? '' : supportingAsset.supportingAssetName) + ' in order to ' + risk.motivation;
            if (allAttackPathsName.length > 0) {
                risk.riskName += `, exploiting the ${allAttackPathsName}`;
            };
        }
    };
}
    ...
    try {
        ...
        if(field){
            if(field === 'threatAgent' || field === 'threatVerb' || field === 'motivation' || field === 'riskName'){
                ...
                else if(field === 'riskName') risk.isAutomaticRiskName = false;
            }else{
                risk[field] = value === 'null' ? null : parseInt(value);
                if(field === 'businessAssetRef') risk.supportingAssetRef = null; 
            };
        };
        ...
        console.log(err);
      dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to update risk ${id}: riskName` });
    }
}; 
...
const updateRiskLikelihood = (israProject, id, field, value, win) =>{
    try {
        ...
        console.log(err);
      dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to update risk ${id}: riskLikelihood` });
    }
};
...
const updateRiskImpact = (israProject, id, field, value, win) => {
    try {
        ...
        if(risk.businessAssetRef){
            const businessAsset = israProject.getBusinessAsset(risk.businessAssetRef);
            const {
...
const addRiskAttackPath = (israProject, riskId, win) => {
    try {
        ...
        console.log(err)
        return dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to add Risk Attack Path for Risk ${riskId}` });
    }
};
...
const deleteRiskAttackPath = (israProject, riskId, ids, win) => {
    try {
        ...
        console.log(err)
        dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to delete Risk Attack Path(s) for Risk ${riskId}` });
    }
};
...
/**
  ...
  * @param {string} field
  * @param {value} value selectedVulnerability
*/
const updateRiskAttackPath = (israProject, riskId, riskAttackPathId, previousVulId, field, value, win) => {
    try {
        const risk = israProject.getRisk(riskId);
        const riskAttackPath = risk.getRiskAttackPath(riskAttackPathId);

        if (field === 'selectedVulnerability') {
            const ref = riskAttackPath.getVulnerability(previousVulId);
            ...
            // Need see if this is necessary since attackPath will only contain valid Vuls
            ...
            for (let i = 0; i < riskAttackPath.properties.vulnerabilityRef.length; i++) {
                if (riskAttackPath.properties.vulnerabilityRef[i].score === null &&
                    riskAttackPath.properties.vulnerabilityRef[i].name !== null) 
                    {
        ...
        console.log(err)
        dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to update Risk Attack Path ${riskAttackPathId} for Risk ${riskId}` });
    }
};
...
/**
  ...
  * @param {integer} riskAttackPathId of selected risk attack path
*/
const addVulnerabilityRef = (israProject, riskId, riskAttackPathId, vulnerabilityId) => {
    try {
        const risk = israProject.getRisk(riskId);
        const riskAttackPath = israProject.getRisk(riskId).getRiskAttackPath(riskAttackPathId);
        const { vulnerabilityName, overallScore } = israProject.getVulnerability(Number(vulnerabilityId));
...
/**
  ...
  * @param {integer} riskId of selected risk
  * @param {integer} riskAttackPathId of selected risk attack path
  * @param {integer} rowIds of selected risk attack path checkbox
*/
const deleteVulnerabilityRef = (israProject, riskId, riskAttackPathId, ids) => {
    try {
        const risk = israProject.getRisk(riskId);
        const riskAttackPath = risk.getRiskAttackPath(riskAttackPathId);

        ids.sort((a, b) => b - a).forEach((vulnerabilityId)=> {
            riskAttackPath.deleteVulnerability(Number(vulnerabilityId));
        });
        calculateAttackPathScore(risk, riskAttackPath);
        calculateAttackPathName(risk, riskAttackPath, israProject);
        ...
        console.log(err)
        dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to delete Risk Attack Path ${riskAttackPathId}'s vulnerabilityId: ${ids} for Risk ${riskId}` });
    }
};
...
const addRiskMitigation = (israProject, riskId, win) => {
    try {
        ...
        console.log(err)
        return dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to add Risk Mitigation for Risk ${riskId}` });
    }
};
...
const deleteRiskMitigation = (israProject, riskId, ids, win) => {
    try {
        ...
        console.log(err)
        return dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to delete Risk Mitigation(s) for Risk ${riskId}` });
    }
};
...
const updateRiskMitigation = (israProject, riskId, riskMitigationId, field, value, win) => {
    try {
        ...
        console.log(err)
        return dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to update Risk Mitigation ${riskMitigationId} for Risk ${riskId}` });
    }
}; 
...
const updateRiskManagement = (israProject, riskId, field, value, win) => {
    try {
        ...
        console.log(err);
        return dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to update Risk Management for Risk ${riskId}` });
    }
};
...
const validateRisks = (israProject, data) => {
    const logRiskId = data && data.riskId;
    if (isLoggingEnabled()) log.info(`[RISK-VALIDATE][backend] riskId=${logRiskId} step=start (mitigations=${data && data.riskMitigation && data.riskMitigation.length})`);
    try {
        const { riskId, riskName, threatAgentDetail, threatVerbDetail, motivationDetail, riskLikelihood, riskMitigation, riskManagementDetail } = data;

        const { riskLikelihoodDetail } = riskLikelihood;
        if (isLoggingEnabled()) log.info(`[RISK-VALIDATE][backend] riskId=${riskId} step=getRisk`);
        const risk = israProject.getRisk(riskId);
        if (isLoggingEnabled()) log.info(`[RISK-VALIDATE][backend] riskId=${riskId} step=assignFields`);
        risk.threatAgentDetail = threatAgentDetail;
        risk.threatVerbDetail = threatVerbDetail;
        risk.motivationDetail = motivationDetail;
        ...
        riskMitigation.forEach((mitigation) => {
            ...
            if (isLoggingEnabled()) log.info(`[RISK-VALIDATE][backend] riskId=${riskId} step=mitigation id=${riskMitigationId}`);
        ...
        if (isLoggingEnabled()) log.info(`[RISK-VALIDATE][backend] riskId=${riskId} step=done <- OK`);
        return israProject.properties.Risk;
    } catch (err) {
        console.error(err)
        if (isLoggingEnabled()) log.error(`[RISK-VALIDATE][backend] riskId=${logRiskId} step=ERROR, about to show blocking dialog: ${err.name}: ${err.message}`, err.stack);
        dialog.showMessageBoxSync(getMainWindow(), { message: 'Failed to validate Risk tab' });
        if (isLoggingEnabled()) log.info(`[RISK-VALIDATE][backend] riskId=${logRiskId} step=dialog-dismissed`);
    }
};
