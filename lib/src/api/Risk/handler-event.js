...
const updateRiskName = (israProject, win, id, field, value) => {
    try {
        ...
        if(field){
            if(field === 'threatAgent' || field === 'threatVerb' || field === 'motivation' || field === 'riskName' || field === 'dependencyOrigin'){
                risk[field] = value;
                if (field === 'threatVerb') updateRiskImpactThreatVerb(israProject, id, value);
                else if(field === 'riskName') risk.isAutomaticRiskName = false;
            }else if(field === 'isTransferredFromDependency'){
                risk[field] = value === 'true' || value === true;
                if (risk[field]) {
                    risk.isAutomaticRiskName = false;
                    risk.riskManagementDecision = 'Transfer';
                }
            }else if(field === 'transferredScore'){
                const score = value === 'null' || value === '' ? null : parseInt(value);
                risk.inherentRiskScore = score;
                risk.mitigatedRiskScore = score;
                risk.residualRiskScore = score;
...
const calculateInherentRiskScore = (risk) => {
    if (risk.isTransferredFromDependency) return;
