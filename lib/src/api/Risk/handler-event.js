...
const calculateMitigationsBenefits = (risk) => {
    const { riskMitigation, inherentRiskScore, riskManagementDecision } = risk.properties;
...
const calculateResidualRiskScore = (risk) => {
    const { riskManagementDecision, inherentRiskScore, mitigationsDoneBenefits } = risk;
...
const updateRiskManagement = (israProject, riskId, field, value, win) => {
    try {
        const risk = israProject.getRisk(riskId);
        risk[field] = value;
        if (risk.mitigatedRiskScore != null) calculateMitigationsBenefits(risk);
        return risk.properties;
    } catch (err) {
        console.log(err);
