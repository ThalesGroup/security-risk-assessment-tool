...
const calculateMitigationsBenefits = (risk) => {
    ...
    if (selectedBenefits.length > 0) risk.mitigationsBenefits = Math.max(...selectedBenefits);
    else risk.mitigationsBenefits = 1;
    if (selectedDoneBenefits.length > 0) risk.mitigationsDoneBenefits = Math.max(...selectedDoneBenefits);
...
const addRiskMitigation = (israProject, riskId) => {
    try {
        ...
        return [[riskMitigation.properties], israProject.properties.Risk];
    } catch (err) {
        console.log(err)
