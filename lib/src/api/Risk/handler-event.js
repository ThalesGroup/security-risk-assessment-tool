...
/**
  ...
  * @param {integer} vulnerabilityId of selected risk attack path vulnerability ref
  * @param {string} field
  * @param {value} value selectedVulnerability
*/
const updateRiskAttackPath = (israProject, riskId, riskAttackPathId, vulnerabilityId, field, value, win) => {
    try {
        const risk = israProject.getRisk(riskId);
        const riskAttackPath = risk.getRiskAttackPath(riskAttackPathId);

        if (field === 'selectedVulnerability') {
            const ref = riskAttackPath.getVulnerability(vulnerabilityId);
            if (value !== '') {
            ...
            riskAttackPath.updateVulnerability(vulnerabilityId, ref);
...
/**
  ...
  * @param {integer} vulnerabilityId of selected risk attack path vulnerability ref
*/
const deleteVulnerabilityRef = (israProject, riskId, riskAttackPathId, vulnerabilityIds, win) => {
    try {
        const risk = israProject.getRisk(riskId);
        const riskAttackPath = risk.getRiskAttackPath(riskAttackPathId);

        vulnerabilityIds.sort((a, b) => b - a).forEach((vulnerabilityId)=> {
            riskAttackPath.deleteVulnerability(vulnerabilityId);
        });
        calculateAttackPathScore(risk, riskAttackPath);
        calculateAttackPathName(risk, riskAttackPath, israProject);
        ...
        dialog.showMessageBoxSync(getMainWindow(), { message: `Failed to delete Risk Attack Path ${riskAttackPathId}'s vulnerabilityId: ${vulnerabilityId} for Risk ${riskId}` });
    }
};
