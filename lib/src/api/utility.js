...
const urlPrompt = async () => {
  ...
  while (url !== null && !url.match(URLpattern)) {
    dialog.showMessageBox(currentWin, { type: 'error', title: 'Invalid URL', message: 'The address is not valid. Enter an address that begins with http://, https://, ftp://, mailto:, or another valid protocol.' });
...
const openUrl = (url, userStatus) => {
  if (userStatus) {
    ...
    win.loadURL(url);
  } else dialog.showMessageBox(currentWin, { type: 'info', title: 'User Status', message: 'You are offline' });
};
...
const saveAsFile = async (base64data, currentFileName) => {
  ...
  if (!fileName.canceled) {
    try {
      ...
      dialog.showMessageBox(currentWin, { message: `Successfully saved file to ${fileName.filePath}` });
    } catch (err) {
      console.log(err);
      dialog.showMessageBox(currentWin, { message: `Error in saving file to ${fileName.filePath}` });
    }
  }
};
