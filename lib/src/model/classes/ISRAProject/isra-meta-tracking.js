...
module.exports = class ISRAMetaTracking {
  ...
  set trackingIteration(trackingIteration) {
    if (Number.isSafeInteger(trackingIteration) || trackingIteration === null) {
      this.#trackingIteration = trackingIteration;
    } else throw new Error(`Tracking iteration ${trackingIteration} is not an integer/null`);
  }
  ...
  set trackingDate(trackingDate) {
    if (typeof trackingDate === 'string' && ISRAMetaTracking.#isValidDate(trackingDate)) this.#trackingDate = trackingDate;
    else throw new Error(`Meta tracking ${this.#trackingIteration}: tracking date is not a string`);
  }
  ...
  static #isValidDate(date) {
    const pattern = /^\d\d\d\d-[0-1]\d-[0-3]\d$/;
