...
const { isValidDate } = require('../../schema/validation-pattern/validation');
...
module.exports = class ISRAMetaTracking {
  ...
  set trackingIteration(trackingIteration) {
    if ((Number.isSafeInteger(trackingIteration)
    && trackingIteration > 0)
  ...
  set trackingDate(trackingDate) {
    if (typeof trackingDate === 'string' && isValidDate(trackingDate)) this.#trackingDate = trackingDate;
