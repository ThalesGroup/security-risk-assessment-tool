...
/** validate targeted level of trust in ISRAProject
    * @param {string} val value intended to be populated into targetedLevelOfTrust class property
    * @return {boolean}
  */
const isTargetedLevelOfTrust = (val) => {
  const subSchema = jsonSchemaISRAmeta.targetedLevelOfTrust;
  return validateClassProperties(val, subSchema);
};
...
/** validate latest businessAssetId in ISRAProject for businessAsset(s)
    * @param {integer} val value intended to be populated into latestBusinessAssetId class property
    * @return {boolean}
  */
const isLatestBusinessAssetId = (val) => {
  const subSchema = jsonSchema.ISRAmeta.properties.latestBusinessAssetId;
  return validateClassProperties(val, subSchema);
};

/** validate latest supportingAssetId in ISRAProject for supportingAsset(s)
    * @param {integer} val value intended to be populated into latestSupportingAssetId class property
    * @return {boolean}
  */
const isLatestSupportingAssetId = (val) => {
  const subSchema = jsonSchema.ISRAmeta.properties.latestSupportingAssetId;
  return validateClassProperties(val, subSchema);
};

/** validate latest supportingAssetId in ISRAProject for risk(s)
    * @param {integer} val value intended to be populated into latestRiskId class property
    * @return {boolean}
  */
const isLatestRiskId = (val) => {
  const subSchema = jsonSchema.ISRAmeta.properties.latestRiskId;
  return validateClassProperties(val, subSchema);
};

/** validate latest vulnerabilityId in ISRAProject for vulnerability(s)
    * @param {integer} val value intended to be populated into latestVulnerabilityId class property
    * @return {boolean}
  */
const isLatestVulnerabilityId = (val) => {
  const subSchema = jsonSchema.ISRAmeta.properties.latestVulnerabilityId;
  return validateClassProperties(val, subSchema);
};
