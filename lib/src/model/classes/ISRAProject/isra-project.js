...
module.exports = class ISRAProject {
  ...
  set projectOrganization(projectOrganization) {
    if (typeof projectOrganization === 'string') this.#projectOrganization = projectOrganization;
    else throw new Error('Project organization is not a string');
  }
  ...
  deleteBusinessAsset(businessAssetId) {
    if (this.#businessAssetsMap.size > 0 && Number.isSafeInteger(businessAssetId)) {
      this.#businessAssetsMap.delete(businessAssetId);
      this.#businessAssetsCount = this.#businessAssetsMap.size;
    } else throw Error(`Business asset ${businessAssetId}: id not an integer`);
  }
  ...
  set supportingAssetsDesc(supportingAssetsDesc) {
    if (typeof supportingAssetsDesc === 'string') this.#supportingAssetsDesc = supportingAssetsDesc;
    else throw new Error('Supporting asset desc is not a string');
  }
  ...
  deleteSupportingAsset(supportingAssetId) {
    if (this.#supportingAssetsMap.size > 0 && Number.isSafeInteger(supportingAssetId)) {
      this.#supportingAssetsMap.delete(supportingAssetId);
      this.#supportingAssetsCount = this.#supportingAssetsMap.size;
    } else throw Error(`Supporting asset ${supportingAssetId}: id not an integer`);
  }
  ...
  deleteRisk(riskId) {
    if (this.#risksMap.size > 0 && Number.isSafeInteger(riskId)) {
      this.#risksMap.delete(riskId);
      this.#risksCount = this.#risksMap.size;
    } else throw Error(`Risk ${riskId}: id not an integer`);
  }
  ...
  deleteVulnerability(vulnerabilityId) {
    if (this.#vulnerabilitiesMap.size > 0 && Number.isSafeInteger(vulnerabilityId)) {
      this.#vulnerabilitiesMap.delete(vulnerabilityId);
      this.#vulnerabilitiesCount = this.#vulnerabilitiesMap.size;
    } else throw Error(`Vulnerability ${vulnerabilityId}: id not an integer`);
  }
  ...
  deleteMetaTracking(trackingIteration) {
    if (this.#metaTrackingMap.size > 0 && Number.isSafeInteger(trackingIteration)) {
    ...
    } else throw Error(`ISRA meta tracking ${trackingIteration}: iteration is not an integer`);
  }
