...
const urlPrompt = async () => {
  ...
  while (url !== null && !url.match(URLpattern)) {
    dialog.showMessageBoxSync(null, { message: 'The address is not valid. Enter an address that begins with http://, https://, ftp://, mailto:, or another valid protocol.' });
...
*/
const saveAsFile = async (base64data) => {
  const options = {
    title: 'Save as file - Electron ISRA Project',
    defaultPath: 'C:\\Users\\File',
  ...
  if (!fileName.canceled) {
    try {
      ...
      fs.writeFileSync(fileName.filePath, content, (err) => {
...
/**
  * Decode file attachments from InfoPath xml
...
const decodeFile = (base64) => {
  if (base64 !== '') {
    ...
    const fileName = buffer.toString('utf16le', 24);
    ...
    return [fileName.match(/.*(json|png|docx|doc|jpg|jpeg|pdf|xml|csv|xlsx|ppt)/)[0], base64data];
  }
