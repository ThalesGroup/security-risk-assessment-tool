...
const { businessAssetEnums } = require('./enum');
...
module.exports = class BusinessAsset {
  ...
  set businessAssetId(businessAssetId) {
    ...
    } else throw new Error(`Business asset id ${businessAssetId} does not match current businessAssetId`);
  }
  ...
  set businessAssetType(businessAssetType) {
    if (typeof businessAssetType === 'string' && businessAssetType in businessAssetEnums) this.#businessAssetType = businessAssetType;
    else throw new Error(`Business asset ${this.#businessAssetId}: type is not a string or is invalid`);
  }

  set businessAssetDescription(businessAssetDescription) {
    if (typeof businessAssetDescription === 'string') this.#businessAssetDescription = businessAssetDescription;
    else throw new Error(`Business asset ${this.#businessAssetId}: description is not a string`);
  }

  set businessAssetProperties(businessAssetProperties) {
    if (businessAssetProperties instanceof BusinessAssetProperties) {
      this.#businessAssetProperties = businessAssetProperties.properties;
    } else throw new Error(`Business asset ${this.#businessAssetId}: Business asset properties is not an object`);
  }
