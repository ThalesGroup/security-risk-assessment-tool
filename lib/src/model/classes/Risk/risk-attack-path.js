...
/**
  ...
  * @throws risk attack path id is invalid null/integer
  * @throws attack path: (riskAttackPathId): attack path name is not a string
  * @throws attack path: (riskAttackPathId): risk attack path is not an object
  * @throws attack path: (riskAttackPathId):
  * Vulnerability (vulnerabilityId) doesn't exist
  * @throws attack path: (riskAttackPathId):
  * Vulnerability (vulnerabilityId): doesn't exist
  * @throws attack path: (riskAttackPathId):
  * attack path score is an invalid null/double
*/
class RiskAttackPath {
  ...
  */
  set riskAttackPathId(riskAttackPathId) {
    ...
    } else throw new Error(`Risk attack path id is invalid null/integer`);
  }
  ...
  */
  set attackPathName(attackPathName) {
    ...
    else throw new Error(`attack path: ${this.#riskAttackPathId}: attack path name is not a string`);
  }
  ...
  */
  addVulnerability(vulnerability) {
    if (!vulnerability.rowId) {
      this.#rowId++;
      vulnerability.rowId = this.#rowId;
    } else this.#rowId = vulnerability.rowId;
    if (isVulnerabilityRef(vulnerability)) {
      this.#vulnerabilityMap.set(vulnerability.rowId, vulnerability);
    } else throw Error(`attack path: ${this.#riskAttackPathId}: risk attack path is not an object`);
  }

  /**
    * delete Vulnerability ref
    * from vulnerabilityMap based on vulnerabilityId
    * @param {integer} rowId - id of Vulnerability Ref Object
    * @param {object} value - Vulnerability Ref Object  
  */
  deleteVulnerability(rowId) {
    if (this.#vulnerabilityMap.has(rowId)) {
      this.#vulnerabilityMap.delete(rowId);
    } else throw Error(`attack path: ${this.#riskAttackPathId}: Vulnerability Ref doesn't exist`);
  }
  ...
  /**
    ...
    * @param {object} value - Vulnerability Ref Object
  */
  updateVulnerability(rowId, value) {
    if (this.#vulnerabilityMap.has(rowId) && isVulnerabilityRef(value)) {
      this.#vulnerabilityMap.set(rowId, value);
    } else throw Error(`attack path: ${this.#riskAttackPathId}: Failed to update Vulnerability Ref`);
  }
  ...
  /** 
    ...
    * @param {integer} rowId - id of Vulnerability Ref Object
  */
  getVulnerability(rowId) {
    if (this.#vulnerabilityMap.has(rowId)) {
      return this.#vulnerabilityMap.get(rowId);
    } throw new Error(`attack path: ${this.#riskAttackPathId}: Vulnerability Ref: doesn't exist`);
  }
  ...
  */
  set attackPathScore(attackPathScore) {
    ...
    else throw new Error(`attack path: ${this.#riskAttackPathId}: attack path score is an invalid null/double`);
  }
