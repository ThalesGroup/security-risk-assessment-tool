...
const deleteRiskAttackPath = (israProject, riskId, ids) => {
    try {
        ...
        return [risk.properties.riskAttackPaths, israProject.properties.Risk];
    } catch (err) {
...
/**
  ...
  * @param {ISRAProject} israProject current ISRA Project
*/
const addVulnerabilityRef = (israProject, riskId, riskAttackPathId) => {
    try {
        ...
        console.log(israProject.getRisk(riskId).getRiskAttackPath(riskAttackPathId).properties)

        return [[riskAttackPath.properties.vulnerabilityRef], israProject.properties.Risk];
    } catch (err) {
        return dialog.showMessageBoxSync(null, { message: `Failed to add Risk Attack Path for Risk ${riskId}` });
    }
};
...
/**
  ...
  * @param {Array} ids of risk attack path id(s)
*/
const deleteVulnerabilityRef = (israProject, riskId, ids) => {
    try {
        const risk = israProject.getRisk(Number(riskId));
        ids.forEach((id) => {
            risk.deleteRiskAttackPath(Number(id));
