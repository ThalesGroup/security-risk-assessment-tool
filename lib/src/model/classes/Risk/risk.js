...
module.exports = class Risk {
  ...
  set riskId(riskId) {
    if ((Number.isSafeInteger(riskId)
    && riskId > 0)
    || riskId === null) this.#riskId = riskId;
    else throw new Error(`Risk id ${riskId} is not null/integer`);
  }
  ...
  deleteRiskAttackPath(riskAttackPathId) {
    ...
    } else throw Error(`Risk ${this.#riskId}: risk attack path doesn't exist`);
  }
  ...
  updateRiskAttackPath(riskAttackPathId, property, value) {
    ...
    } else throw Error(`Risk ${this.#riskId}: risk attack path ${riskAttackPathId}: deson't exist`);
  }
  ...
  deleteRiskMitigation(riskMitigationId) {
    ...
    } else throw Error(`Risk ${this.#riskId}: risk mitigation doesn't exist`);
  }
  ...
  set riskManagementDecision(riskManagementDecision) {
    if (riskManagementDecision in riskManagementDecisionEnum) {
  ...
  set riskManagementDetail(riskManagementDetail) {
    if (typeof riskManagementDetail === 'string' && isValidHtml(riskManagementDetail)) this.#riskManagementDetail = riskManagementDetail;
    else throw new Error(`Risk ${this.#riskId}: management detail is not a html string`);
  }
  ...
  set residualRiskLevel(residualRiskLevel) {
    if (residualRiskLevel in residualRiskLevelEnum) this.#residualRiskLevel = residualRiskLevel;
