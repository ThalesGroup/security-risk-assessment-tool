...
module.exports = class Risk {
  ...
  deleteRiskAttackPath(riskAttackPathId) {
    ...
    } else throw Error(`Risk ${this.#riskId}: risk attack path id is invalid`);
  }
  ...
  updateRiskAttackPath(riskAttackPathId, property, value) {
    ...
    } else throw Error(`Risk ${this.#riskId}: risk attack path ${riskAttackPathId}: is invalid`);
  }
  ...
  deleteRiskMitigation(riskMitigationId) {
    ...
    && this.#riskMitigationMap.size > 0
    && this.#riskAttackPathMap.has(riskMitigationId)) {
    ...
    } else throw Error(`Risk ${this.#riskId}: risk mitigation id is not an integer or doesn't exist`);
  }

  // updates RiskMitigation based on their riskMitigationId
  updateRiskMitigation(riskMitigationId, property, value) {
    if (Number.isSafeInteger(riskMitigationId) && this.#riskAttackPathMap.has(riskMitigationId)) {
      const riskMitigation = this.#riskAttackPathMap.get(riskMitigationId);
      riskMitigation[property] = value;
    } else throw Error(`Risk ${this.#riskId}: risk attack path ${riskMitigationId}: is not an integer or doesn't exist`);
  }
