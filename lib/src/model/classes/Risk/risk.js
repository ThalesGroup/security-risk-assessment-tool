...
class Risk {
  ...
  constructor() {
    ...
    this.#isTransferredFromDependency = riskJsonSchema.isTransferredFromDependency.default;
    this.#dependencyOrigin = riskJsonSchema.dependencyOrigin.default;
  ...
  */
  ...
  /** verify if risk is transferred from a dependency
    * @type {boolean}
  */
  set isTransferredFromDependency(boolean) {
    if (isTransferredFromDependency(boolean)) {
      this.#isTransferredFromDependency = boolean;
    } else throw new Error(`Risk ${this.#riskId}: value is not a boolean`);
  }

  get isTransferredFromDependency() {
    return this.#isTransferredFromDependency;
  }

  /** comment for its origin / Dependency Name
    * @type {string}
  */
  set dependencyOrigin(string) {
    if (isDependencyOrigin(string)) {
      this.#dependencyOrigin = string;
    } else throw new Error(`Risk ${this.#riskId}: dependency origin is not a string`);
  }
  ...
  */
  get properties() {
    return {
      ...
      ...(this.#isTransferredFromDependency ? {
        isTransferredFromDependency: this.#isTransferredFromDependency,
