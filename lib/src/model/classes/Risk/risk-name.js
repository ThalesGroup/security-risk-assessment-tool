...
const { threatAgentEnum, threatVerbEnum } = require('./enum');
...
module.exports = class RiskName {
  ...
  set threatAgent(threatAgent) {
    if (typeof threatAgent === 'string' && threatAgent in threatAgentEnum) this.#threatAgent = threatAgent;
    else throw new Error(`Risk ${this.#riskIdRef}: threat agent is not an string or is invalid`);
  }
  ...
  set threatVerb(threatVerb) {
    if (typeof threatVerb === 'string' && threatVerb in threatVerbEnum) this.#threatVerb = threatVerb;
    else throw new Error(`Risk ${this.#riskIdRef}: threat verb is not an string or is invalid`);
  }
