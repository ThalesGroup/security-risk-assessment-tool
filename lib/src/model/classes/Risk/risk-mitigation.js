...
module.exports = class RiskMitigation {
  ...
  set riskMitigationId(riskMitigationId) {
    if (Number.isSafeInteger(riskMitigationId) || riskMitigationId === null) {
      this.#riskMitigationId = riskMitigationId;
    } else throw new Error(`Risk ${riskMitigationId}: Risk id ref is not an integer`);
  }

  set description(description) {
    if (typeof description === 'string') this.#description = description;
    else throw new Error(`Risk ${this.#riskIdRef}: attack path: ${this.#riskMitigationId}: description is not a string`);
  }

  set benefits(benefits) {
    if (!Number.isNaN(benefits) || benefits === null) this.#benefits = benefits;
    else throw new Error(`Risk ${this.#riskIdRef}: ${this.#riskMitigationId}: benefits is not a double`);
  }

  set cost(cost) {
    if (!Number.isSafeInteger(cost) || cost === null) this.#cost = cost;
    else throw new Error(`Risk ${this.#riskIdRef}: ${this.#riskMitigationId}: cost is not an integer`);
  }

  set decision(decision) {
    if (typeof decision === 'string') this.#decision = decision;
    else throw new Error(`Risk ${this.#riskIdRef}: ${this.#riskMitigationId}: decision is not a string`);
  }

  set decisionDetail(decisionDetail) {
    if (typeof decisionDetail === 'string') this.#decisionDetail = decisionDetail;
  ...
  set mitigationsBenefits(mitigationsBenefits) {
    if (!Number.isNaN(mitigationsBenefits) || mitigationsBenefits === null) {
      this.#mitigationsBenefits = mitigationsBenefits;
    } else throw new Error(`Risk ${this.#riskIdRef}: ${this.#riskMitigationId}: mitigations benefits is not a double`);
  }

  set mitigationsDoneBenefits(mitigationsDoneBenefits) {
    if (!Number.isNaN(mitigationsDoneBenefits) || mitigationsDoneBenefits === null) {
      this.#mitigationsDoneBenefits = mitigationsDoneBenefits;
    } else throw new Error(`Risk ${this.#riskIdRef}: ${this.#riskMitigationId}: mitigations done benefits is not a double`);
  }
