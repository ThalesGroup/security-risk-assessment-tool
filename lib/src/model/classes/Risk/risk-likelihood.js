...
module.exports = class RiskLikelihood {
  ...
  set riskLikelihood(riskLikelihood) {
    if (Number.isSafeInteger(riskLikelihood) || riskLikelihood === null) {
      this.#riskLikelihood = riskLikelihood;
    } else throw new Error(`Risk ${this.#riskIdRef}: risk likelihood is not an integer`);
  }
  ...
  set skillLevel(skillLevel) {
    if (Number.isSafeInteger(skillLevel) || skillLevel === null) this.#skillLevel = skillLevel;
    else throw new Error(`Risk ${this.#riskIdRef}: skill level is not an integer`);
  }

  set reward(reward) {
    if (Number.isSafeInteger(reward) || reward === null) this.#reward = reward;
    else throw new Error(`Risk ${this.#riskIdRef}: reward is not an integer`);
  }

  set accessResources(accessResources) {
    if (Number.isSafeInteger(accessResources) || accessResources === null) {
      this.#accessResources = accessResources;
    } else throw new Error(`Risk ${this.#riskIdRef}: access resources is not an integer`);
  }

  set size(size) {
    if (Number.isSafeInteger(size) || size === null) this.#size = size;
    else throw new Error(`Risk ${this.#riskIdRef}: size is not an integer`);
  }

  set intrusionDetection(intrusionDetection) {
    if (Number.isSafeInteger(intrusionDetection) || intrusionDetection === null) {
      this.#intrusionDetection = intrusionDetection;
    } else throw new Error(`Risk ${this.#riskIdRef}: intrusion detection is not an integer`);
  }
  ...
  set threatFactorLevel(threatFactorLevel) {
    if (typeof threatFactorLevel === 'string') this.#threatFactorLevel = threatFactorLevel;
    else throw new Error(`Risk ${this.#riskIdRef}: threat factor level is not a string`);
  }

  set occurrence(occurrence) {
    if (Number.isSafeInteger(occurrence) || occurrence === null) this.#occurrence = occurrence;
    else throw new Error(`Risk ${this.#riskIdRef}: occurrence is not an integer`);
  }

  set occurrenceLevel(occurrenceLevel) {
    if (typeof occurrenceLevel === 'string') this.#occurrenceLevel = occurrenceLevel;
