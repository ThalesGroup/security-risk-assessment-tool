...
describe('Populate class values (valid & invalid)', () => {
  ...
  test('set projectURL', () => {
    ...
    // old URLpattern expects a real-looking host, so avoid single-label host like "server"
    projectContext.projectURL = 'ftp://server.com/pathname';
    expect(projectContext.projectURL).toBe('ftp://server.com/pathname');
    ...
    const testValues = [
      ...
      'www.google.com',
      'mailto:invalidEmail',
      'http:///', // regression for the xml "http:///" case
    ];
  ...
  test('set vulnerabilityTrackingURI', () => {
    ...
    // old URLpattern expects a real-looking host, so avoid single-label host like "server"
    vulnerability.vulnerabilityTrackingURI = 'ftp://server.com/pathname';
    expect(vulnerability.vulnerabilityTrackingURI).toBe('ftp://server.com/pathname');
      ...
      Risk: [
        {
          ...
          supportingAssetRef: null,
          isAutomaticRiskName: true,
          isTransferredFromDependency: false,
          dependencyOrigin: '',
          riskLikelihood: {
            riskLikelihood: 4,
            riskLikelihoodDetail: '',
