...
const { isValidHtml } = require('../../src/model/schema/validation-pattern/validation');
...
describe('XML is partially filled, class values are valid', () => {
  ...
  test('Test each value in test-2.xml', async () => {
    await XML2JSON('./lib/test/test-7/test-7.xml', israProject);
    ...
    expect(ISRAmeta.ISRAtracking[1]).toEqual(israTracking2);
    expect(ISRAmeta.ISRAtracking[2]).toEqual(israTracking3);
    ...
    // Test Supporting Asset Desc
    expect(SupportingAssetsDesc).toBe('');

    // Test Supporting Asset
    const sa1 = {
      supportingAssetId: 1,
