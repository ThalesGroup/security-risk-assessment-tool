...
describe('Test DataStore, DataLoad, DataNew', () => {
  ...
  test('Reject DataStore ', () => {
    expect.assertions(1);
    const invalidFilePath = path.join(os.tmpdir(), `no-such-dir-${Date.now()}`, 'file.sra');
    return expect(DataStore(israProject, invalidFilePath)).rejects.toEqual(new Error('Failed to save file'));
  });
