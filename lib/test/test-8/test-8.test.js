...
describe('Populate class values (valid & invalid)', () => {
  ...
  test('delete metaTracking', () => {
    ...
    israProject.getMetaTracking(2).trackingComment = 'comment 2';
    israProject.getMetaTracking(3).trackingComment = 'comment 3';
    israProject.deleteMetaTracking(2);

    expect(israProject.getMetaTracking(1).properties).toEqual({
      trackingIteration: 1,
      trackingSecurityOfficer: '',
      trackingDate: '',
      trackingComment: 'comment',
    ...
    expect(israProject.getMetaTracking(2).properties).toEqual({
      trackingIteration: 2,
      trackingSecurityOfficer: user,
      trackingDate: currentDate,
      trackingComment: 'comment 3',
    });
    ...
    expect(() => {
      israProject.getMetaTracking(2);
    }).toThrow();

    israProject.deleteMetaTracking(1);

    expect(() => {
      israProject.getMetaTracking(1);
    }).toThrow();
  });
});
